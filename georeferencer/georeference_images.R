...
alpha = ns67.joined$Pitch
gamma = ns67.joined$Roll
beta = ns67.joined$Yaw
...
#headings[X_h > 0 && Y_h > 0] <- 360 - headings[X_h > 0 && Y_h > 0]
...
georeference <-
  data.frame(
    ...
    altitude = ns67.joined$Altitude_m - 175.5,
...
georeference$ll <-
  destPoint(
    p = as.matrix(georeference[c("lon", "lat")]),
    b = georeference$bearing - 90,
    d = georeference$altitude * tan(hfov / 2 * pi / 180)
  )
...
orthagonal_distance_ground <-
  georeference$altitude * tan(vfov * pi / 180)

# get direct distances
orthagonal_distance_direct <-
  sqrt(orthagonal_distance_ground ^ 2 + georeference$altitude ^ 2)

corner_distance_direct <-
  orthagonal_distance_direct / cos(hfov / 2 * pi / 180)

corner_distance_ground <-
  sqrt(corner_distance_direct^2 - georeference$altitude^2)

angle_to_corner <- acos(orthagonal_distance_ground / corner_distance_ground) * 180 / pi

# calculate upper left corner
georeference$ul <-
  destPoint(
    p = as.matrix(georeference[c("lon", "lat")]),
    b = georeference$bearing - angle_to_corner,
